...
fields1=('code-sequence 1','code-sequence 2','code-sequence 3','code-sequence 4','sender','reciever')
...
def final1(entries):
    m=entries['code-sequence 1'].get()
    if(m==0 or m==1 or m==-1):
        c1.append(m)
    m=entries['code-sequence 2'].get()
    if(m==0 or m==-1 or m==1):
        c2.append(m)
    m=entries['code-sequence 3'].get()
    if(m==0 or m==-1 or m==1):
        c3.append(m)
    m=entries['code-sequence 4'].get()
    if(m==0 or m==-1 or m==1):
        c4.append(m)
    ...
    l1=[same_3_neighbors[0]*x for x in c1]
    l2=[same_3_neighbors[1]*x for x in c2]
    l3=[same_3_neighbors[2]*x for x in c3]
    l4=[same_3_neighbors[3]*x for x in c4]
...
def func1(l,m):
    ...
    ans=p//4
    if(ans>0):
        messagebox.showinfo("The data sent was 1")
    elif(ans<0):
        messagebox.showinfo("The data sent was 0")
    else:
        messagebox.showinfo("Silent i.e no data sent")
